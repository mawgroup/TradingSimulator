...
//							if(slice.bestPriceCount==slice.bestPrices.length)
			...
			if(0<this.trader.getInputStream().available()){
				...
				switch(method){
					...
					case "orderComplete": orderComplete(is.readInt());break;
					case "orderPartiallyComplete": orderPartiallyComplete(is.readInt());break;
				}
			}
	...
	private void newOrder(int clientId, int clientOrderId, NewOrderSingle nos) throws IOException{
		orders.put(id, new Order(clientId, clientOrderId, nos.instrument, nos.size, nos.pos));
	...
	private void internalCross(int id, Order o) throws IOException{
		for(Map.Entry<Integer, Order>entry:orders.entrySet()){
			...
			if(matchingOrder.instrument.toString().equals(o.instrument.toString()) && matchingOrder.initialMarketPrice == o.initialMarketPrice && !matchingOrder.pos.equals(o.pos)) {
				//TODO add support here and in Order for limit orders
				int sizeBefore = o.sizeRemaining();
				System.out.println("After IF CROSS");
				System.out.println("Match ID: " + matchingOrder.clientOrderID + " Match size: " + matchingOrder.size);
				System.out.println("Original ID: " + o.clientOrderID + " Original size: " + o.size);
				...
				if (sizeBefore != o.sizeRemaining()) {
					...
					System.out.println("Match ID: " + matchingOrder.clientOrderID + " Match size: " + matchingOrder.sizeRemaining());
					System.out.println("Original ID: " + o.clientOrderID + " Original size: " + o.sizeRemaining());
				}
			}
		}
	}
	...
	private void orderComplete(int id) throws IOException{
		int cID = orders.get(id).clientid;
		ObjectOutputStream os=new ObjectOutputStream(clients[cID].getOutputStream());
		os.writeObject("11=0;35=F;39=F");
		os.writeObject(orders.get(id));
		os.flush();
		orders.remove(id);
	}
	private void orderPartiallyComplete(int id) throws IOException{
		int cID = orders.get(id).clientid;
		ObjectOutputStream os=new ObjectOutputStream(clients[cID].getOutputStream());
		os.writeObject("11=0;35=P;39=F");
		os.flush();
	}
