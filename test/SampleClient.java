...
import static Database.Database.writeSentOrder;
...
public class SampleClient extends Mock implements Client{
	...
	public int sendOrder(Object par0)throws IOException{
		...
		int instid= 0;   //RANDOM_NUM_GENERATOR.nextInt(3);

		/*
		Here we try to create the buy sell
		 */

		int posNr=RANDOM_NUM_GENERATOR.nextInt(2);
		String pos;
		if(posNr == 1)
			pos = "buy";
		else
		...
		NewOrderSingle nos=new NewOrderSingle(size,instid,instrument,pos);

		//log order sent
		writeSentOrder(nos);

		show("sendOrder: id="+id+" size="+size+" instrument="+INSTRUMENTS[instid].toString() + " position=" + pos);
	...
	enum methods{newOrderSingleAcknowledgement,dontKnow, orderCompleted, orderPartiallyCompleted};
	@Override
			...
			while(true){
				...
				while(0<omConn.getInputStream().available()){
					...
					for(int i=0;i<fixTags.length;i++){
						...
						switch(tag_value[0]){
							...
							case"35":MsgType=tag_value[1].charAt(0);
								...
								if(MsgType=='F') {
									whatToDo = methods.orderCompleted;
									order = (Order)is.readObject();
								}
								if(MsgType=='P')whatToDo=methods.orderPartiallyCompleted;
								break;
					...
					switch(whatToDo){
						...
						case orderCompleted:orderCompleted(order);break;
						case orderPartiallyCompleted:orderPartiallyCompleted();break;
					}
	...
	void orderCompleted(Order o){
		System.out.println(Thread.currentThread().getName()+" acknowledged Order Is Complete");
		OUT_QUEUE.remove(o.clientOrderID);
	}
	void orderPartiallyCompleted(){
		System.out.println(Thread.currentThread().getName()+" acknowledged Order Is Partially Complete");
	}
